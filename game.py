...
class game:
    def __init__(self, screen):
        ...
        self.cameraX = 0
        self.map = 1
        self.listHitbox = [pygame.Rect((0, 479), (backgrounds[self.map].get_width(), 161)), pygame.Rect((0, 415), (plateforme2.get_width(), plateforme2.get_height()))]
        self.plateform = [surface(pygame.Rect((0, 415), (plateforme2.get_width(), plateforme2.get_height())), plateforme2)]
        self.menu = 1
        self.menuTimer = 0
    ...
    def displayMenu(self, mouse):
        ...
        if self.menu == 1:
            ...
            menu = label(self.screen, 200, 50, 400, 50, "Menu")
            menu.pygamePrint()
            ...
            if not self.menuTimer:
                if mouse[0] and startgameButton.x <= pygame.mouse.get_pos()[0] <= startgameButton.x+startgameButton.width \
                        and startgameButton.y <= pygame.mouse.get_pos()[1] <= startgameButton.y+startgameButton.height:
                    self.menu = False
                elif mouse[0] and settingsButton.x <= pygame.mouse.get_pos()[0] <= settingsButton.x + settingsButton.width \
                        and settingsButton.y <= pygame.mouse.get_pos()[1] <= settingsButton.y + settingsButton.height:
                    self.menu = 2
                    self.menuTimer = 30
            ...
            if not self.menuTimer:
                if mouse[0] and player1RightButton.x <= pygame.mouse.get_pos()[0] <= player1RightButton.x+player1RightButton.width \
                        and player1RightButton.y <= pygame.mouse.get_pos()[1] <= player1RightButton.y + player1RightButton.height:
                    ...
                    while not keyFind:
                        for event in pygame.event.get():
                            if event.type == pygame.KEYDOWN:
                                ...
                                keyFind = True
                elif mouse[0] and player1LeftButton.x <= pygame.mouse.get_pos()[0] <= player1LeftButton.x+player1LeftButton.width \
                        and player1LeftButton.y <= pygame.mouse.get_pos()[1] <= player1LeftButton.y + player1LeftButton.height:
                    ...
                    while not keyFind:
                        for event in pygame.event.get():
                            if event.type == pygame.KEYDOWN:
                                ...
                                keyFind = True
                elif mouse[0] and player1JumpButton.x <= pygame.mouse.get_pos()[0] <= player1JumpButton.x+player1JumpButton.width \
                        and player1JumpButton.y <= pygame.mouse.get_pos()[1] <= player1JumpButton.y + player1JumpButton.height:
                    ...
                    while not keyFind:
                        for event in pygame.event.get():
                            if event.type == pygame.KEYDOWN:
                                ...
                                keyFind = True
                elif mouse[0] and player1ThrowButton.x <= pygame.mouse.get_pos()[0] <= player1ThrowButton.x+player1ThrowButton.width \
                        and player1ThrowButton.y <= pygame.mouse.get_pos()[1] <= player1ThrowButton.y + player1ThrowButton.height:
    ...
    def changeMap(self, nextMap, player):
        ...
        if player == 1:
            ...
            self.cameraX = -(backgrounds[self.map].get_width() - 800)
        if len(self.listHitbox) > 1:
            self.listHitbox = [self.listHitbox[0]]
        if self.map in [0, 1, 2, 3]:
    ...
    def displayBg(self):
        ...
        self.screen.blit(backgrounds[self.map], (self.cameraX, 0))
        if self.plateform != []:
    ...
    def displaySword(self):
        for s in self.sword_list:
            if s.throw == (1, "left", 1) or s.throw == (1, "right", 1) or s.throw == (1, "left", 2) or s.throw == (1, "right", 2):
    ...
    def keyboardInput(self, key):
        if key[pygame.K_ESCAPE]:
            ...
            if self.p1.x >= 750 and self.map < len(backgrounds)-1:
                self.changeMap(1, 1)
            self.p2.movePlayer(key, self.p1, self.listHitbox)
            if self.p2.x <= 5 and self.map > 0:
                self.changeMap(-1, 2)

    def testKillPlayer(self):
        if self.p1.sword is not None and pygame.Rect.colliderect(self.p2.hitbox, self.p1.sword.hitbox) and not self.p2.timingRespawn and not self.p1.timingRespawn:
            self.swordNumber = self.p2.dieP(600, 415, 0, self.sword_list, self.swordNumber)
        elif self.p2.sword is not None and pygame.Rect.colliderect(self.p1.hitbox, self.p2.sword.hitbox) and not self.p1.timingRespawn and not self.p2.timingRespawn:
            self.swordNumber = self.p1.dieP(200, 415, 1, self.sword_list, self.swordNumber)

    def swordInteractions(self):
        for s in self.sword_list:
            if s.throw == (1, "left", 2):
                if self.p1.sword is not None and pygame.Rect.colliderect(s.hitbox, self.p1.sword.hitbox) and not self.p1.timingRespawn:
                ...
                if self.p2.sword is not None and pygame.Rect.colliderect(s.hitbox, self.p2.sword.hitbox) and not self.p2.timingRespawn:
                    s.stopThrow(swordRight)
            ...
            elif self.p2.sword is None and pygame.Rect.colliderect(s.hitbox, self.p2.hitbox) and not self.p2.timerpickUp:
    ...
    def changeCamera(self):
        ...
        if -(backgrounds[self.map].get_width() - 800) <= self.cameraX - move <= 0:
            self.cameraX -= move
            if move:
